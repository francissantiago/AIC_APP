...
export class EbdPage extends BasePage {
  ...
  async gotoReports(): Promise<void> {
    ...
    await waitForAppShell(this.page);
    await this.page
      .getByTestId("class-frequency-report")
      .waitFor({ state: "visible" });
  }
  ...
  async openAttendance(classId: string): Promise<void> {
    ...
    await this.page
      .getByTestId("class-attendance-panel")
      .waitFor({ state: "visible" });
  }
  ...
  async markMemberPresent(memberId: string): Promise<void> {
    await this.page
      .locator(`input[name="present-${memberId}"]`)
      .first()
      .check();
  }
  ...
  async filterReport(classId: string, from: string, to: string): Promise<void> {
    await this.page
      .getByTestId("class-report-class-select")
      .selectOption(classId);
    await this.page.locator("#class-frequency-report-from").fill(from);
    await this.page.locator("#class-frequency-report-to").fill(to);
