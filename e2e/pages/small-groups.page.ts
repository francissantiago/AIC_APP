...
export class SmallGroupsPage extends BasePage {
  ...
  async goto(): Promise<void> {
    ...
    await this.page
      .getByTestId("small-group-create-btn")
  ...
  async gotoReports(): Promise<void> {
    ...
    await waitForAppShell(this.page);
    await this.page
      .getByTestId("small-group-frequency-report")
      .waitFor({ state: "visible" });
  }
  ...
  async openMembers(groupId: string): Promise<void> {
    ...
    await this.page
      .getByTestId("small-group-members-panel")
      .waitFor({ state: "visible" });
  }
  ...
  async addMember(memberId: string): Promise<void> {
    await this.page
      .getByTestId("small-group-add-member-select")
      .selectOption(memberId);
    await this.page.getByTestId("small-group-add-member-btn").click();
    await this.page
      .getByTestId(`small-group-member-row-${memberId}`)
      .waitFor({ state: "visible" });
  }

  async removeMember(memberId: string): Promise<void> {
    await this.page
      .getByTestId(`small-group-member-remove-${memberId}`)
      .click();
  ...
  async openMeetings(groupId: string): Promise<void> {
    ...
    await this.page
      .getByTestId("small-group-meetings-panel")
      .waitFor({ state: "visible" });
  }
  ...
  async openMeetingAttendance(meetingId: string): Promise<void> {
    await this.page
      .getByTestId(`small-group-meeting-attendance-${meetingId}`)
      .click();
    await this.page
      .getByTestId("small-group-attendance-panel")
      .waitFor({ state: "visible" });
  }

  async markMemberPresent(memberId: string): Promise<void> {
    await this.page
      .locator(`input[name="sg-present-${memberId}"]`)
      .first()
      .check();
  }
  ...
  async filterReport(groupId: string, from: string, to: string): Promise<void> {
    await this.page
      .getByTestId("small-group-report-group-select")
      .selectOption(groupId);
    await this.page.locator("#small-group-frequency-report-from").fill(from);
    await this.page.locator("#small-group-frequency-report-to").fill(to);
