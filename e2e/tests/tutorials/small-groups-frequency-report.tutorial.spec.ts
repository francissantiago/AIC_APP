...
import { e2eSmallGroupName, monthRangeIsoDate } from '../../helpers/test-data.helper';
...
test.describe('small-groups-frequency-report tutorial', () => {
  ...
  test('small-groups-frequency-report — relatório por período', async ({ page, tutorialStep }) => {
    ...
    await tutorialStep('Abrir relatório de frequência de células', async () => {
      ...
      await expect(page.getByTestId('small-group-frequency-report')).toBeVisible();
    });
