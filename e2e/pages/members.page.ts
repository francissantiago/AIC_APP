...
export class MembersPage extends BasePage {
  ...
  async fillCreateForm(options: {
    ...
    if (options.birthDate) {
      await this.page
        .getByTestId("member-form-birth-date")
        .fill(options.birthDate);
    }

    if (options.status) {
      await this.page
        .getByTestId("member-form-status")
  ...
  async startTransferWizard(): Promise<void> {
    ...
    await this.page
      .getByTestId("member-transfer-wizard")
      .waitFor({ state: "visible" });
  }
  ...
  async transferGoToPreview(): Promise<void> {
    ...
    await this.page
      .getByTestId("transfer-letter-preview")
      .waitFor({ state: "visible" });
  }
