...
function AddtoHome(time, once) {
    if (once) {
        var AddHomeStatus = localStorage.getItem("AicappAddtoHome");
        if (AddHomeStatus === "1" || AddHomeStatus === 1) {
            // already showed up
        }
        else {
            localStorage.setItem("AicappAddtoHome", 1)
...
//-----------------------------------------------------------------------
// Dark Mode
var checkDarkModeStatus = localStorage.getItem("AicappDarkmode");
var switchDarkMode = document.querySelectorAll(".dark-mode-switch");
var pageBodyActive = pageBody.classList.contains("dark-mode");

// Check if enable as default
if (Aicapp.Dark_Mode.default) {
    pageBody.classList.add("dark-mode");
}

// Local Dark Mode
if (Aicapp.Dark_Mode.local_mode.enable) {
    var nightStart = Aicapp.Dark_Mode.local_mode.start_time;
    var nightEnd = Aicapp.Dark_Mode.local_mode.end_time;
...
if (Aicapp.Dark_Mode.auto_detect.enable)
...
switchDarkMode.forEach(function (el) {
    el.addEventListener("click", function () {
        var darkmodeCheck = localStorage.getItem("AicappDarkmode");
        var bodyCheck = pageBody.classList.contains('dark-mode');
        if (darkmodeCheck === 1 || darkmodeCheck === "1" || bodyCheck) {
            pageBody.classList.remove("dark-mode");
            localStorage.setItem("AicappDarkmode", "0");
            switchDarkModeCheck(false);
        }
        else {
            pageBody.classList.add("dark-mode")
            switchDarkModeCheck(true);
            localStorage.setItem("AicappDarkmode", "1");
        }
    })
})
//-----------------------------------------------------------------------
...
function testMode() {
    ...
    console.clear();
    console.log("%cAICAPP", "font-size: 1.3em; font-weight: bold; color: #FFF; background-color: #6236FF; padding: 10px 120px; margin-bottom: 16px;")
    ...
    testModeMsg(Aicapp.PWA.enable, "PWA")
    testModeMsg(Aicapp.Dark_Mode.default, "Set dark mode as default theme")
    testModeMsg(Aicapp.Dark_Mode.local_mode.enable, "Local dark mode (between " + Aicapp.Dark_Mode.local_mode.start_time + ":00 and " + Aicapp.Dark_Mode.local_mode.end_time + ":00)")
    testModeMsg(Aicapp.Dark_Mode.auto_detect.enable, "Auto detect dark mode")
    testModeMsg(Aicapp.RTL.enable, "RTL")
    testModeMsg(Aicapp.Test.enable, "Test mode")
    testModeMsg(Aicapp.Test.alert, "Test mode alert")
    ...
    testModeMsg(Aicapp.Animation.goBack, "Go Back")
}
function themeTesting() {
    var word = Aicapp.Test.word;
    var value = "";
