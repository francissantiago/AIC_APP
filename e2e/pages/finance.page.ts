...
export class FinanceDashboardPage extends BasePage {
  ...
  async filterPeriod(from: string, to: string): Promise<void> {
    ...
    await this.page
      .getByTestId("finance-dashboard-cards")
      .waitFor({ state: "visible" });
  }
}
...
export class FinanceEntriesPage extends BasePage {
  ...
  async openCategoriesDialog(): Promise<void> {
    ...
    await this.page
      .getByTestId("finance-category-manager")
      .waitFor({ state: "visible" });
  }
  ...
  async fillEntryForm(options: {
    ...
    await this.page
      .getByTestId("finance-entry-form-type")
    ...
    await this.page
      .getByTestId("finance-entry-form-description")
      .fill(options.description);
    await this.page
      .getByTestId("finance-entry-form-amount")
      .fill(options.amount);
    if (options.memberId) {
      ...
      if (options.memberSearch) {
        ...
        await expect(
          memberSelect.locator(`option[value="${options.memberId}"]`),
        ).toHaveCount(1, {
          timeout: 15_000,
        });
      }
  ...
  async saveEntryForm(): Promise<void> {
    ...
    await this.page
      .locator("dialog[open]")
      .filter({ has: this.page.getByTestId("finance-entry-form") })
      .waitFor({ state: "hidden" });
  }
...
export class AssetsPage extends BasePage {
  ...
  async fillAssetForm(name: string, acquisitionValue = "1000"): Promise<void> {
    ...
    await this.page
      .getByTestId("asset-form-acquisition-value")
      .fill(acquisitionValue);
  }
...
export class FinanceReportsPage extends BasePage {
  ...
  async applyCashPeriod(from: string, to: string): Promise<void> {
    await this.page.locator("#finance-reports-cash-from").fill(from);
    await this.page.locator("#finance-reports-cash-to").fill(to);
    await this.page
      .getByRole("button", { name: /^Filtrar$|^Filter$/i })
      .first()
      .click();
    await this.page
      .getByTestId("finance-reports-cash-table")
      .waitFor({ state: "visible" });
  }
  ...
  async exportCashCsv(): Promise<void> {
    const responsePromise = this.page.waitForResponse(
      (response) =>
        response.url().includes("/finance/reports/cash-flow.csv") &&
        response.ok(),
    );
