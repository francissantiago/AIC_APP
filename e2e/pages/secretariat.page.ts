...
export class SecretariatDashboardPage extends BasePage {
  ...
  async goto(): Promise<void> {
    ...
    await this.page
      .getByTestId("secretariat-dashboard")
...
export class AgendaCalendarPage extends BasePage {
  ...
  async fillEventForm(options: {
    ...
    await this.fillDatetimeInput("agenda-form-starts", options.startsAt);
    await this.fillDatetimeInput("agenda-form-ends", options.endsAt);
  }

  private async fillDatetimeInput(
    dateTestId: string,
    isoValue: string,
  ): Promise<void> {
    const [datePart, timePart] = isoValue.split("T");
    const dateInput = this.page.getByTestId(dateTestId);
    await dateInput.fill(datePart);
    const timeInput = this.page
      .locator(
        `app-datetime-input:has(input[data-testid="${dateTestId}"]) input[type="text"]`,
      )
      .nth(1);
  ...
  async clickCalendarEvent(
    ...
    await event.click();
    await this.page
      .getByTestId("agenda-event-preview")
      .waitFor({ state: "visible" });
  ...
  async deleteCurrentEvent(): Promise<void> {
    ...
    if (await previewDelete.isVisible().catch(() => false)) {
      ...
      await this.page
        .getByRole("button", { name: /^Excluir$|^Delete$|^Eliminar$/i })
        .click();
    }
    const confirmDialog = this.page.getByRole("dialog", {
      name: /Tem certeza|Are you sure|Confirm/i,
    });
    await confirmDialog.getByTestId("dialog-confirm").click();
    await confirmDialog.waitFor({ state: "hidden" });
    await this.page
      .getByTestId("agenda-form")
      .waitFor({ state: "hidden" })
      .catch(() => undefined);
    await this.page
      .getByTestId("agenda-event-preview")
...
export class VisitorsPage extends BasePage {
  ...
  async fillVisitorForm(options: {
    fullName: string;
    visitDate: string;
  }): Promise<void> {
    await this.page
      .getByTestId("visitor-form-full-name")
      .fill(options.fullName);
    await this.page
      .getByTestId("visitor-form-visit-date")
      .fill(options.visitDate);
  }
  ...
  async openConvert(id: string): Promise<void> {
    ...
    await this.page
      .getByTestId("visitor-convert-form")
      .waitFor({ state: "visible" });
  }
...
export class AttendancePage extends BasePage {
  ...
  async fillAttendanceForm(options: {
    ...
    if (options.eventType) {
      await this.page
        .getByTestId("attendance-form-type")
        .selectOption(options.eventType);
    }
    await this.page
      .getByTestId("attendance-form-total")
      .fill(options.totalPresent);
  }
...
export class DocumentsPage extends BasePage {
  ...
  async fillDocumentForm(options: {
    title: string;
    documentDate: string;
  }): Promise<void> {
    await this.page.getByTestId("document-form-title").fill(options.title);
    await this.page
      .getByTestId("document-form-date")
      .fill(options.documentDate);
  }
  ...
  async saveDocumentForm(mode: "create" | "update" = "update"): Promise<void> {
    ...
    if (mode === "create") {
      await this.page
        .getByTestId("document-form-upload-input")
  ...
  async uploadPdfInForm(): Promise<void> {
    ...
    const uploadResponse = this.page.waitForResponse(
      (response) =>
        response.url().includes("/secretariat/documents/") &&
...
export class SchedulesBoardPage extends BasePage {
  ...
  async openEditorForEvent(eventId: string): Promise<void> {
    await this.eventCard(eventId)
      .getByRole("button", { name: /Editar escala|Editor/i })
      .click();
    await this.page
      .getByTestId("schedule-editor-form")
      .waitFor({ state: "visible" });
  }
  ...
  async fillAssignmentForEvent(options: {
    ...
    await this.page
      .getByTestId("schedule-editor-ministry")
      .selectOption(options.ministryId);
    await membersLoad;
    await this.page
      .getByText(/Carregando|Loading/i)
      .waitFor({ state: "hidden" })
    ...
    await this.page
      .getByTestId("schedule-editor-member")
      .last()
      .selectOption(options.memberId);
    await this.page
      .getByTestId("schedule-editor-role-label")
      .last()
      .fill(options.roleLabel);
  }
